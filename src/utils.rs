...
fn gen_keypair() {
    ...
    let public_key = general_purpose::STANDARD.encode(pk);
    let secret_key = general_purpose::STANDARD.encode(sk);
    println!("Public Key:  {public_key}");
    println!("Secret Key:  {secret_key}");
}

fn validate_keypair(pk: &str, sk: &str) -> ResultType<()> {
    let sk1 = general_purpose::STANDARD.decode(sk);
    if sk1.is_err() {
    ...
    let pk1 = general_purpose::STANDARD.decode(pk);
    if pk1.is_err() {
...
fn doctor_ip(server_ip_address: std::net::IpAddr, server_address: Option<&str>) {
    ...
    match lookup_addr(&server_ip_address) {
        ...
        Err(e) => {
            println!("Reverse DNS lookup failed: {e}");
        }
    }
...
fn doctor(server_address_unclean: &str) {
    ...
    if let Ok(server_ipaddr) = server_address.parse::<IpAddr>() {
        ...
        let ips = match lookup_host(server_address) {
            Ok(ips) => ips,
            Err(e) => {
                println!("DNS lookup failed for '{server_address}': {e}");
                return;
            }
        };
